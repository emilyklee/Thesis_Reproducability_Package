...
def run_drgep(model_file, importance_coeffs, ignition_conditions, psr_conditions, flame_conditions, 
    ...
    # Set up logging file
    # logging.getLogger('GRI_Mech3_reduction_data')
    # logging.FileHandler('GRI_Mech3_reduction_data.log')
    # logging.basicConfig(filename='GRI_Mech3_reduction_data.log',level=logging.DEBUG, filemode='w')
...
if __name__ == '__main__':
    ...
    # Comparison of DRGEP to the New Mthod for the GRI Mech 3 model
    
    # General information for both methods
    # input .yaml filename
    # use 'gri30_drgep_nosa_1.yaml' for reproducing plot and 'gri30_drgep_nosa.yaml' for table data
    # 'gri30_drgep_nosa_1.yaml' only contains one ignition condition 
    yaml_file = 'gri30_drgep_nosa.yaml'
    ...
    threshold_data_new_method, num_species_data_new_method, error_data_new_method = run_drgep(model_file, new_method_overall_interaction_coeffs, ignition_conditions, psr_conditions, 
                                                                                                   flame_conditions, error_limit, species_targets, species_safe, phase_name, upper_threshold_new_method, num_threads, path)
    ...
    with PdfPages('Plots/GRI_Mech_3_plot.pdf') as pdf:
        fig,ax1 = plt.subplots(figsize=(10,8))
        ax1.grid(linestyle='--')
        ax1.plot(threshold_data_drgep, num_species_data_drgep, 'o', color = colors[1], markersize=10, label='DRGEP: Number of species')
        ax1.plot(threshold_data_new_method, num_species_data_new_method, 's', color = colors[7], markersize=10, label='Modified DRGEP: Number of species')
        ax1.set_ylabel('Number of species')
        ax1.set_xlabel('Threshold')
        ax1.set_xlim([0,0.32])
        ax1.set_ylim(top=45)
        ax1.annotate('# of species: Modified DRGEP', xy=(0.075,33),
            xytext=(0.16, 33), fontsize=14,
            arrowprops=dict(facecolor='black',arrowstyle='-'),
            horizontalalignment='center', verticalalignment='center'
            )
        ax1.annotate('# of species: DRGEP', xy=(0.145,22),
            xytext=(0.08, 22), fontsize=14,
            arrowprops=dict(facecolor='black',arrowstyle='-'),
            horizontalalignment='center', verticalalignment='center'
            )
        ax2 = ax1.twinx()
        ax2.plot(threshold_data_drgep, error_data_drgep, 'D', color = colors[0], markersize=10, label='DRGEP: Error')
        ax2.plot(threshold_data_new_method, error_data_new_method, '^', color = colors[6], markersize=10, label='Modified DRGEP: Error')
        ax2.set_ylabel('Error (%)')
        ax2.set_xlim([0,0.32])
        ax2.annotate('Error: Modified DRGEP', xy=(0.295,88),
            xytext=(0.26, 70), fontsize=14,
            arrowprops=dict(facecolor='black',arrowstyle='-'),
            horizontalalignment='center', verticalalignment='center'
            )
        ax2.annotate('Error: DRGEP', xy=(0.22,88),
            xytext=(0.22, 81), fontsize=14,
            arrowprops=dict(facecolor='black',arrowstyle='-'),
            horizontalalignment='center', verticalalignment='center'
            )
        # ax2.set_ylim([0,10])
        ...
        ax1.xaxis.set_tick_params(which='minor', size=7, width=2, direction='out')
        ax1.yaxis.set_tick_params(which='major', size=10, width=2, direction='out')
        ax1.yaxis.set_tick_params(which='minor', size=7, width=2, direction='out')
        ax2.xaxis.set_tick_params(which='major', size=10, width=2, direction='out')
        ax2.xaxis.set_tick_params(which='minor', size=7, width=2, direction='out')
        ax2.yaxis.set_tick_params(which='major', size=10, width=2, direction='out')
        ax2.yaxis.set_tick_params(which='minor', size=7, width=2, direction='out')
